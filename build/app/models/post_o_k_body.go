...
type PostOKBody struct {
	...
	// ssh
	SSH []*PostOKBodySSHItems `json:"ssh"`
}
...
func (m *PostOKBody) validateSSH(formats strfmt.Registry) error {
	if swag.IsZero(m.SSH) { // not required
		return nil
	}

	for i := 0; i < len(m.SSH); i++ {
		if swag.IsZero(m.SSH[i]) { // not required
			continue
		}

		if m.SSH[i] != nil {
			if err := m.SSH[i].Validate(formats); err != nil {
				if ve, ok := err.(*errors.Validation); ok {
					return ve.ValidateName("ssh" + "." + strconv.Itoa(i))
				} else if ce, ok := err.(*errors.CompositeError); ok {
					return ce.ValidateName("ssh" + "." + strconv.Itoa(i))
				}
				return err
			}
		}
...
func (m *PostOKBody) contextValidateSSH(ctx context.Context, formats strfmt.Registry) error {

	for i := 0; i < len(m.SSH); i++ {

		if m.SSH[i] != nil {
			if err := m.SSH[i].ContextValidate(ctx, formats); err != nil {
				if ve, ok := err.(*errors.Validation); ok {
					return ve.ValidateName("ssh" + "." + strconv.Itoa(i))
				} else if ce, ok := err.(*errors.CompositeError); ok {
					return ce.ValidateName("ssh" + "." + strconv.Itoa(i))
